...
def GetArticleInformation(TOCurl, ArticleTableName, TitleTableName, AuthorTableName, PageNoTableName, AbstractTableName, KeywordTableName, DOITableName):
	...
	# Article Title
	   ...
	   # urlTags = soup.findAll("a", {"class": TitleTableName})
		...
		for idx, item in enumerate(title):
			...
			AbstractDescription = soup_details(
				"div", {"class": AbstractTableName})
			DOIName1 = ''
			if DOITableName != "":
				...
				# print(DOIName)
				for DOI1 in DOIName:
					DOIName1 = DOI1.replace(
						'<a href="https://doi.org/', '').replace('">', '')
				# print(DOIName1)
  ...
  # Dummy To test
			...
			Abstract = ((str(AbstractDescription[0].text).replace(
				"Abstract", "", 1)).replace('\n', '').replace('\t', ''))
			Keyword = soup_details("div", {"class": KeywordTableName})
			KeywordDescription = ((str(Keyword[0].text).replace(
				"Keywords", "", 1)).replace('\n', '').replace('\t', ''))
...
# Regular Expression
			...
			hn.append({'Articletitle': titleName, 'DOI': DOIName1, 'Page': PageNo, 'link': href,
					   'Author': AuthorName, 'Abstract': Abstract, 'Keywords': KeywordDescription})
...
GetArticleInformation('https://journals.scholarpublishing.org/index.php/TMLAI/issue/view/287',
					  'obj_article_summary', 'title', 'authors', 'pages', 'item abstract', 'item keywords', 'item doi')
