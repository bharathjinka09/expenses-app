...
def tree_data(request):
    ...
    for name in os.listdir(rootNode):
        ...
        if os.path.isfile(fullpath):
            ...
            response += [
                {"id": relativePath, "text": name, "url": relativePath, "cls": "folder"}
            ]
    ...
    if request.POST.get("cmd") == "getMeta":
        return HttpResponse(
            open(
                os.path.join(
                    IDE_PATH,
                    "metafiles/.%s"
                    % urllib.parse.unquote_plus(request.POST.get("app_name")),
                ),
                "a+",
            ).read(),
        ...
        try:
            if request.POST.get("meta") != None:
                ...
                metaArrayClient = json.loads(request.POST.get("meta"))
        ...
        if keywords and len(keywords) > 0:
            ...
            index_path_file = os.path.join(
                IDE_PATH, "metafiles/.%s_index" % urllib.parse.unquote_plus(app_name)
            )
            ...
            return HttpResponse(
                json.dumps(arrResults)
                if len(arrResults) > 0
                else json.dumps([["No results found", ""]])
            )
    else:
        return HttpResponse("")
...
def setMetaFile(app_name, metaData):
    ...
    metaDataFile = os.path.join(
        IDE_PATH, "metafiles/.%s" % urllib.parse.unquote_plus(app_name)
    )
    try:
        handle = open(metaDataFile, "w")
        handle.write(metaData)
    except IOError as errno:
        return HttpResponse("File exception (%s): %s, %s" % (metaDataFile, errno))
    finally:
        handle.close()
